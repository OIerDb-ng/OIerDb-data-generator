...
def get_longlat_baidu(location):
    res = requests.get(f'https://api.map.baidu.com/geocoder?output=json&address={location}&ak={BAIDU_API_KEY}')
    res.encoding = 'utf8'
    try:
...
def get_longlat_google(location):
    while True:
        try:
            res = requests.get(f'https://maps.googleapis.com/maps/api/geocode/json?address={quote(location)}&key={GOOGLE_API_KEY}')
        except Exception:
            continue
        res.encoding = 'utf8'
        try:
